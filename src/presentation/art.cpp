...
bool load_pixel(sf::Texture& texture, const std::string& path,
                bool repeating, const sf::IntRect& area = {}) {
...
bool Art::load(const std::string& asset_dir) {
    ...
    if (!load_pixel(pipe, asset_dir + "/Tiles/Style 5/PipeStyle5.png", true,
                    sf::IntRect({0, 0}, {kPipeTileW, kPipeTileH}))) {
        return false;
    }
